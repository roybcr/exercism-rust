...
// With that in mind, we can say that for any score x, if x > 2^n, the allergy n, must be included in x.

// STEPS TO SOLVE:
// ----------------------------------------------------------------------------------
// As long as score > 0, subtract from score 2^n where n = log2(score) rounded down.
// if score > 2^7 (the allergy with the largest score), continue doing the step above.
// else, include the allergy (represented by 2^n) in the list of allergies.
