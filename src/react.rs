...
#[derive(Debug)]
pub struct Generator(Cell<u32>);

#[allow(dead_code)]
impl Generator {
    fn new() -> Self { Generator(Cell::from(0)) } 
...
impl Hash for CellId {
    fn hash<H: Hasher>(&self, state: &mut H) {
        match self {
            CellId::Input(InputCellId(i)) => i.to_string().hash(state),
            CellId::Compute(ComputeCellId(c)) => c.to_string().hash(state),
        };
    }
}
...
type Callback<'a> = Box<(dyn Fn<(&'a [CellId],),Output=CellId> + 'static)>;
fn box_callback<'a, F>(func: F) -> Callback<'a> 
    where F: Fn<(&'a [CellId],),Output=CellId> + 'static {
    Box::new(func) as Callback
}
...
pub struct Reactor<'a, T> {
   ...
   pub observables: HashMap<CellId, T>,
   pub subscribers: HashMap<CellId, Callback<'a>>,
   pub subscriptions: HashMap<CellId, HashMap<CellId, T>>,
}
...
impl<'a, T: Copy + PartialEq> Reactor<'a, T> {
    pub fn new() -> Self { 
        Reactor::<T> {
            ...
            observables: HashMap::<CellId, T>::new(),
            subscribers: HashMap::<CellId, Callback<'a>>::new(),
            subscriptions: HashMap::<CellId,HashMap<CellId, T>>::new(),
        } 
    }
    ...
    pub fn create_input(&mut self, initial: T) -> InputCellId { 
        ...
        let input_cell_id = InputCellId(self.generator.gen());
        let cell_id = CellId::Input(input_cell_id);
        self.observables.insert(cell_id, initial);
        input_cell_id

    }
    
    // Creates a compute cell with the specified dependencies and compute
    // function. The compute function is expected to take in its arguments
    // in the same order as specified in `dependencies`.
    // You do not need to reject compute functions that expect more arguments
    // than there are dependencies (how would you check for this, anyway?).
    //
    ...
    // This means that you may assume, without checking, that if the
    // dependencies exist at creation time they will continue to exist as
    // long as the Reactor exists.
    pub fn create_compute<F: Fn(&[CellId]) -> CellId>(&mut self,
                                            dependencies: &[CellId],
                                            compute_func: F)
                                            -> Result<ComputeCellId, CellId> {

        let compute_cell_id = ComputeCellId(self.generator.gen());
        ...
        self.subscribers.insert(cell_id, box_callback(compute_func))
    ...
    // exist.
    //
    // You may wonder whether it is possible to implement `get(&self, id:
    // CellId) -> Option<&Cell>` and have a `value(&self)` method on `Cell`.
    //
    // It turns out this introduces a significant amount of extra complexity to
    // this exercise. We chose not to cover this here, since this exercise
    // is probably enough work as-is.
    ...
    //
    // Similarly, you may wonder about `get_mut(&mut self, id: CellId) ->
    // Option<&mut Cell>`, with a `set_value(&mut self, new_value: T)` method on
    // `Cell`.
    //
    ...
    pub fn remove_callback(&mut self,
        ...
        unimplemented!("Remove the callback identified by the CallbackId {:?} from the \
                        cell {:?}",
                       callback,
                       cell,)
    }
...
mod tests {
    ...
    fn compute_cells_calculate_initial_value() {
        ...
        let output =
            reactor.create_compute(&[CellId::Input(input)], |v| v[0] + 1).unwrap();
    ...
    fn compute_cells_take_inputs_in_the_right_order() {
        ...
        let output = reactor.create_compute(&[CellId::Input(one), CellId::Input(two)],
                                            |v| v[0] + v[1] * 10)
                            .unwrap();
    ...
    fn error_creating_compute_cell_if_input_doesnt_exist() {
        ...
        assert_eq!(Reactor::new().create_compute(&[CellId::Input(input)], |_| 0),
    ...
    fn do_not_break_cell_if_creating_compute_cell_with_valid_and_invalid_input() {
        ...
        assert_eq!(reactor.create_compute(&[CellId::Input(input),
    ...
    fn compute_cells_can_depend_on_other_compute_cells() {
        ...
        let input = reactor.create_input(1);
        let times_two =
            reactor.create_compute(&[CellId::Input(input)], |v| v[0] * 2).unwrap();
        let times_thirty =
            reactor.create_compute(&[CellId::Input(input)], |v| v[0] * 30).unwrap();
        let output = reactor.create_compute(&[CellId::Compute(times_two),
    ...
    fn error_adding_callback_to_nonexistent_cell() {
        ...
        let input = dummy_reactor.create_input(1);
        let output =
            dummy_reactor.create_compute(&[CellId::Input(input)], |_| 0).unwrap();

        assert_eq!(Reactor::new().add_callback(output, |_: u32| { println!("hi") }),
                   None);
    }
    ...
    fn error_removing_callback_from_nonexisting_cell() {
        ...
        let _ =
            dummy_reactor.create_compute(&[CellId::Input(dummy_input)], |_| 0).unwrap();
        let dummy_output =
            dummy_reactor.create_compute(&[CellId::Input(dummy_input)], |_| 0).unwrap();
        ...
        assert_eq!(reactor.remove_callback(dummy_output, callback),
    ...
    fn callbacks_only_fire_on_change() {
        ...
        let mut reactor = Reactor::new();
        let input = reactor.create_input(1);
        let output = reactor.create_compute(&[CellId::Input(input)], |v| {
                                if v[0] < 3 {
                                    111
                                } else {
                                    222
    ...
    fn removing_a_callback_multiple_times_doesnt_interfere_with_other_callbacks() {
        ...
        for _ in 1..5 {
            assert_eq!(reactor.remove_callback(output, callback),
    ...
    fn callbacks_should_only_be_called_once_even_if_multiple_dependencies_change() {
        ...
        let minus_one1 =
            reactor.create_compute(&[CellId::Input(input)], |v| v[0] - 1).unwrap();
        let minus_one2 =
            reactor.create_compute(&[CellId::Compute(minus_one1)], |v| v[0] - 1).unwrap();
        let output = reactor.create_compute(&[CellId::Compute(plus_one),
    ...
    fn callbacks_should_not_be_called_if_dependencies_change_but_output_value_doesnt_change(
        ...
        let always_two = reactor.create_compute(&[CellId::Compute(plus_one),
    ...
    fn test_adder_with_boolean_values() {
        ...
        let a_xor_b = reactor.create_compute(&[CellId::Input(a), CellId::Input(b)],
                                             |v| v[0] ^ v[1])
                             .unwrap();

        let sum = reactor.create_compute(&[CellId::Compute(a_xor_b),
                                           CellId::Input(carry_in)],
                                         |v| v[0] ^ v[1])
                         .unwrap();

        let a_xor_b_and_cin = reactor.create_compute(&[CellId::Compute(a_xor_b),
                                                       CellId::Input(carry_in)],
                                                     |v| v[0] && v[1])
                                     .unwrap();

        let a_and_b = reactor.create_compute(&[CellId::Input(a), CellId::Input(b)],
                                             |v| v[0] && v[1])
                             .unwrap();

        let carry_out = reactor.create_compute(&[CellId::Compute(a_xor_b_and_cin),
                                                 CellId::Compute(a_and_b)],
                                               |v| v[0] || v[1])
                               .unwrap();

        let tests = &[(false, false, false, false, false),
        ...
        for &(aval, bval, cinval, expected_cout, expected_sum) in tests {
            ...
            assert_eq!(reactor.value(CellId::Compute(carry_out)),
                       Some(expected_cout));
        }
    }
}
